...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Каленадрь.ЕдиницаПериодическогоВарианта = ТипЕдиницыШкалыВремени.Час;
	Каленадрь.КратностьПериодическогоВарианта = 24;
	
	Каленадрь.ОтступСКонцаПереносаШкалыВремени = 3;
	Каленадрь.ОтступСНачалаПереносаШкалыВремени = 8;
	
	Каленадрь.ВыравниватьГраницыЭлементовПоШкалеВремени = Ложь;
	
	Каленадрь.ТекущиеПериодыОтображения.Очистить();
	НачалоПериода = НачалоНедели(ТекущаяДатаСеанса());
	КонецПериода = КонецНедели(ТекущаяДатаСеанса());
	НовыйЭлементПериод = Каленадрь.ТекущиеПериодыОтображения.Добавить(НачалоПериода, КонецПериода);
...
Процедура ЗаполнитьЗанятия()
	...
	Пока Выборка.Следующий() Цикл
		...
		НовыйЭлемент = Каленадрь.Элементы.Добавить(ДатаНачала, ДатаОкончания);
		НовыйЭлемент.Текст = Выборка.ГрафикРаботы;
		НовыйЭлемент.Значение = Выборка.ГрафикРаботы;
